...
from .version import __VERSION__ as myro_version
...
def import_file(filename):
    ...
    globalspath, f = __file__.rsplit(os.sep, 1)
...
class SerialRobot:
    def __init__(self, serialport=None, baudrate=38400):
        from jyro.robot import ask
        if serial is None:
            raise Exception("WARNING: pyserial not loaded: can't upgrade robot!")
        self.robotinfo = {}
        if serialport == None:
            serialport = ask("Port")
